...
const (
    ...
    <p>Computes basic statistics for a giver list of numbers</p>`
...
func homePage(writer http.ResponseWriter, request *http.Request) {
	...
	if err != nil {
		...
		if numbers, message, ok := processRequest(request); ok {
			...
			log.Printf("[%v] serverd OK", request.RemoteAddr)
		} else if message != "" {
			fmt.Fprintf(writer, anError, message)
			log.Printf("[%v] bad request : %v", request.RemoteAddr, message)
		}
	}
